...
def render_minimap():
    ...
    for entity in entities:
        if entity.active:
            ...
            pygame.draw.circle(map_surface, color, (entity_map_x, entity_map_y), 2)
...
def next_level():
    global current_level, MAP, entities, player_pos, player_angle, player_ammo, enemies_killed, total_enemies
...
def check_level_complete():
    ...
    active_enemies = sum(1 for entity in entities if entity.type == "enemy" and entity.active)
...
while running:
    ...
    if len(music_pattern) > 0 and not level_complete:
    ...
    # Рендеринг и проверка завершения уровня
